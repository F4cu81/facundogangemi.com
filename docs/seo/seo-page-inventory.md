...
| Key internal links | Article cards (dynamic `<a href="/insights/{slug}/">` per published article), `/contact/` (Final CTA primary), `/advisory/` (Final CTA secondary), `/about/` (Final CTA proof text), `/case-studies/` (Final CTA proof text — Phase 9 internal linking). **Note (O-1 verified 2026-05-14):** `/insights/ai-strategy/`, `/insights/digital-transformation/`, `/insights/operational-excellence/` and `/insights/ai-adoption/` exist as real crawlable Astro hub routes but are **not linked from this page via `<a href>`**. The category filter controls are `<button type="button" data-filter-chip="…">` elements — JavaScript-only, not crawlable. Category hub pages are reachable via sitemap, article BreadcrumbList and footer navigation. |
| Improvement notes | Phase 6a: hero with editorial image, filter chips, featured insight card, topic cards, article grid. Title fixed from 87-char overage to 57 chars. Meta description trimmed to 127 chars. Internal /about link added to Final CTA section. CollectionPage JSON-LD added. Phase 10 (2026-05-14): Strategy Execution category replaced with AI Adoption; Strategic Perspectives topic cards section removed; filter section always visible; article cards carry data attributes for client-side filtering. Phase 10D/10E (2026-05-14): first production article published in AI Strategy hub; article image support in Featured card and Latest grid; 3-column desktop article layout (sticky TOC, article body, Latest Insights sidebar); search results section added below filters; article template renamed from `[slug].astro` to `[...slug].astro` to support nested slugs. **Future enhancement:** add crawlable `<a href>` links to each category hub from `/insights/` (e.g. a topic nav strip or category label links on filter chips) to establish a proper internal link signal from the hub to its category pages. |
